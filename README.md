...
The self-signed local identity alone does not guarantee prompt-free updates: macOS file-based Keychain partition ACLs can remain bound to individual code hashes. This was reproduced with existing NotchTalk items after an Always Allow approval. Use an Apple-issued signing identity with a stable team identity for the next validation; do not weaken Keychain ACLs to suppress prompts. Existing keychain items are not deleted or rewritten by installation. macOS microphone/accessibility permissions may also need approval after the identity changes.
